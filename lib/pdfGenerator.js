...
async function trackPDFUsage(provider) {
  try {
    ...
    if (providerIndex) {
      const currentCount = parseInt(existingRow[providerIndex] || 0);
      const newCount = currentCount + 1;
      
      // Update the specific cell
      await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: `PDFUsage!${String.fromCharCode(65 + providerIndex)}${monthRowIndex + 1}`,
        valueInputOption: 'USER_ENTERED',
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: `PDFUsage!E${monthRowIndex + 1}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: { values: [[nzTimestamp]] }
      });
      
      // Log usage with quota warnings
      const adobeCount = provider === 'Adobe' ? newCount : parseInt(existingRow[1] || 0);
      const pdfshiftCount = provider === 'PDFShift' ? newCount : parseInt(existingRow[2] || 0);
      const api2pdfCount = provider === 'API2PDF' ? newCount : parseInt(existingRow[3] || 0);
      
      let logMessage = `[PDF COUNTER] ${monthKey} → Adobe: ${adobeCount}/500, PDFShift: ${pdfshiftCount}/50, API2PDF: ${api2pdfCount} (paid)`;
      
      // Add quota warnings
      if (adobeCount >= 450) {
        logMessage += '\n⚠️ Adobe quota nearly reached (limit 500)';
      }
      if (pdfshiftCount >= 45) {
        logMessage += '\n⚠️ PDFShift quota nearly reached (limit 50)';
      }
