...
export async function generateQuotePDF(quoteData) {
  ...
  // Financial data - extract from cost breakdown if available
  let labourTotal = 0;
  ...
  // Try to extract from cost breakdown first
  if (data.cost && data.cost.breakdown) {
    console.log('📊 Extracting financial data from cost breakdown:', data.cost.breakdown);
    
    data.cost.breakdown.forEach(item => {
      const total = parseFloat(item.total) || 0;
      const serviceName = (item.service || '').toLowerCase();
      
      if (serviceName.includes('labour')) {
        labourTotal = total;
      } else if (serviceName.includes('material')) {
        materialsTotal = total;
      } else if (serviceName.includes('travel')) {
        travelTotal = total;
      } else if (serviceName.includes('setup') || serviceName.includes('installation')) {
        installationTotal = total;
      }
    });
    
    subtotal = parseFloat(data.cost.subtotal) || 0;
    gst = parseFloat(data.cost.gst) || 0;
    finalTotal = parseFloat(data.cost.total) || 0;
  } else {
    ...
    finalTotal = parseFloat(data.total) || (subtotal + gst);
  }
