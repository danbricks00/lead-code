  ...
  if (req.method === 'GET') {
    try {
      ...
      if (currentStatus === 'accepted') {
        // Get the timestamp when the quote was accepted (from the first column)
        ...
        try {
          ...
          const quoteRowIndex = rows.findIndex(row => row[1] === quoteId || row[2] === quoteNumber);
             ...
             await sheets.spreadsheets.values.update({
               ...
               resource: { values: [['accepted', currentTime]] }
             });
             console.log('✅ Quote status updated to accepted with timestamp');
           }
        } catch (sheetsError) {
      ...
      const html = `
        ...
        <body>
            <div class="success">
                ...
                <p><strong>Quote Number:</strong> ${quoteNumber}</p>
                <p><strong>Accepted on:</strong> ${currentTime}</p>
