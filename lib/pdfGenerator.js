...
function normalizeLabourData(data) {
  ...
  const rateSources = [
    ...
    // Room-level labour rates (average all rooms)
    ...(data.rooms || []).map(room => room.labourRate || room.laborRate || room.hourlyRate || 0),
  ...
  const hoursSources = [
    ...
    // Calculate from rooms if available
    ...(data.rooms || []).map(room => room.labourHours || room.laborHours || room.hours || 0),
  ...
  const totalSources = [
    ...
    ...(data.rooms || []).map(room => room.labourCost || room.laborCost || room.workCost || 0),
...
function normalizeMaterialsData(data) {
  ...
  const costSources = [
    ...
    // Room-level materials (sum all rooms)
    ...(data.rooms || []).map(room => room.materialsCost || room.materialCost || 0),
  ...
  const quantitySources = [
    ...
    // Calculate from rooms if available
    ...(data.rooms || []).map(room => room.sqm || 0),
