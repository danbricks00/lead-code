...
function selectPDFProvider(usage) {
  ...
  // PDFShift: max 50 / month
  if (pdfshiftCount < 50) {
    console.log(`🎯 PDFShift available: ${pdfshiftCount}/50 - using PDFShift`);
    return 'PDFShift';
  } else {
...
async function convertHTMLToPDF(html) {
  ...
  const usage = await getCurrentPDFUsage();
  const selectedProvider = selectPDFProvider(usage);
  ...
  console.log(`🎯 Selected provider: ${selectedProvider} (based on current usage)`);
  
  // Try selected provider first, then fallback to others if it fails
  const providers = [];
  
  if (selectedProvider === 'Adobe' && process.env.ADOBE_PDF_CLIENT_ID && process.env.ADOBE_PDF_CLIENT_SECRET) {
    providers.push('Adobe');
  }
  if (selectedProvider === 'PDFShift' && process.env.PDFSHIFT_API_KEY) {
    providers.push('PDFShift');
  }
  if (selectedProvider === 'API2PDF' && process.env.API2PDF_API_KEY) {
    providers.push('API2PDF');
  }
  
  // Add fallback providers in order (excluding the selected one)
  if (selectedProvider !== 'Adobe' && process.env.ADOBE_PDF_CLIENT_ID && process.env.ADOBE_PDF_CLIENT_SECRET && usage.adobeCount < 500) {
    providers.push('Adobe');
  }
  if (selectedProvider !== 'PDFShift' && process.env.PDFSHIFT_API_KEY && usage.pdfshiftCount < 50) {
    providers.push('PDFShift');
  }
  if (selectedProvider !== 'API2PDF' && process.env.API2PDF_API_KEY) {
    providers.push('API2PDF');
  ...
  // Add HTML/CSS to PDF as final fallback
  if (process.env.HTMLCSSTOPDF_API_KEY) {
    providers.push('HTMLCSSTOPDF');
  }
    ...
    try {
      ...
      switch (provider) {
        ...
        case 'API2PDF':
          ...
          break;
        case 'HTMLCSSTOPDF':
          result = await convertWithHTMLCSSTOPDF(html);
          break;
        default:
          continue;
      }
...
async function convertWithAdobePDF(html) {
  ...
  try {
    ...
    // Create PDF from HTML using the correct Adobe PDF Services API endpoint
