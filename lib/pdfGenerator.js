...
// Reliable PDF generation solution using multiple providers
...
async function trackPDFUsage(provider) {
  try {
    ...
    if (providerIndex && usage.monthRowIndex >= 0) {
      ...
      let logMessage = `[PDF COUNTER] ${usage.monthKey} → PDFShift: ${pdfshiftCount}/50, API2PDF: ${api2pdfCount}`;
...
function formatDate(date) {
  ...
  // If it's already a formatted string (like "13/09/2025"), return it as is
...
<body>
    <div class="quote-container">
        ...
        <!-- Room Details -->
        <div class="section room-details" style="display: none;">
        ...
        <div class="breakdown-section" style="margin-bottom: 30px;">
            ...
            <!-- Project Summary -->
            <div style="background: linear-gradient(135deg, #e8f4f8 0%, #f0f8ff 100%); padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #667eea; display: none;">
        ...
        <!-- Footer -->
        <div class="footer">
            <p>Thank you for choosing Heat.nz for your underfloor heating needs.</p>
...
export async function generateQuotePDF(quoteData) {
  console.log('🔄 PDF Generation started with data:', JSON.stringify(quoteData, null, 2));

  // Handle both flat and nested data structures
  let data;
  if (quoteData.pdfData) {
    // New structure from quote-submit.js
    data = {
      ...quoteData,
      ...quoteData.pdfData,
      // Flatten nested objects
      customerName: quoteData.pdfData.customer?.name || quoteData.customerName,
      customerEmail: quoteData.pdfData.customer?.email || quoteData.customerEmail,
      customerPhone: quoteData.pdfData.customer?.phone || quoteData.customerPhone,
      customerAddress: quoteData.pdfData.customer?.address || quoteData.customerAddress,
      TradePersonName: quoteData.pdfData.tradesperson?.name || quoteData.TradePersonName,
      tradePersonEmail: quoteData.pdfData.tradesperson?.email || quoteData.tradePersonEmail,
      tradePersonPhone: quoteData.pdfData.tradesperson?.phone || quoteData.tradePersonPhone,
      tradePersonLicense: quoteData.pdfData.tradesperson?.license || quoteData.tradePersonLicense,
      // Financial data
      totalSqm: quoteData.pdfData.project?.totalSqm || quoteData.totalSqm,
      TOTAL_SQM: quoteData.pdfData.project?.TOTAL_SQM || quoteData.totalSqm,
      subtotal: quoteData.pdfData.cost?.subtotal || quoteData.subtotal,
      gst: quoteData.pdfData.cost?.gst || quoteData.gst,
      total: quoteData.pdfData.cost?.total || quoteData.totalQuote,
      // Breakdown data
  ...
  console.log('📊 Processed data for PDF:', JSON.stringify(data, null, 2));
    
    // Replace placeholders with actual data
  let html = getQuoteTemplate();

  // Customer details - use multiple fallback sources
  html = html.replace(/{{CUSTOMER_NAME}}/g, data.customerName || data.CustomerName || 'N/A');
  html = html.replace(/{{CUSTOMER_EMAIL}}/g, data.customerEmail || data.CustomerEmail || 'N/A');
  html = html.replace(/{{CUSTOMER_PHONE}}/g, data.customerPhone || data.CustomerPhone || 'N/A');
  // Show only suburb and area, not full street address
  const addressParts = (data.customerAddress || data.Address || '').split(',');
  const suburbArea = addressParts.length >= 2 ? 
    `${addressParts[addressParts.length - 2]?.trim()}, ${addressParts[addressParts.length - 1]?.trim()}` : 
    (data.customerAddress || data.Address || 'N/A');
  html = html.replace(/{{CUSTOMER_ADDRESS}}/g, suburbArea);
  html = html.replace(/{{SERVICE_TYPE}}/g, data.serviceType || data.ServiceType || 'Underfloor Heating');

  // Tradesperson details
  html = html.replace(/{{TRADESPERSON_NAME}}/g, data.TradePersonName || 'N/A');
  html = html.replace(/{{TRADESPERSON_EMAIL}}/g, data.tradePersonEmail || 'N/A');
  html = html.replace(/{{TRADESPERSON_PHONE}}/g, data.tradePersonPhone || 'N/A');
  html = html.replace(/{{TRADESPERSON_LICENSE}}/g, data.tradePersonLicense || 'N/A');

  // Quote details
  html = html.replace(/{{QUOTE_ID}}/g, data.quoteId || 'N/A');
  html = html.replace(/{{QUOTE_DATE}}/g, formatDate(data.date || new Date()));
  html = html.replace(/{{VALID_UNTIL}}/g, formatDate(data.validUntil));

  // Financial data - extract from top-level fields first (from quote-submit.js)
  let labourTotal = 0;
  ...
  // Extract from top-level fields first (these are the correct values from quote-submit.js)
  labourTotal = parseFloat(data.labourTotal) || 0;
  materialsTotal = parseFloat(data.materialsTotal) || 0;
  travelTotal = parseFloat(data.travelTotal) || 0;
  installationTotal = parseFloat(data.installationCost) || 0;
  subtotal = parseFloat(data.subtotal) || (labourTotal + materialsTotal + travelTotal + installationTotal);
  gst = parseFloat(data.gst) || (subtotal * 0.15);
  finalTotal = parseFloat(data.totalQuote) || parseFloat(data.total) || (subtotal + gst);

  console.log('💰 PDF Generator - Using top-level financial data:', {
    labourTotal,
    materialsTotal,
    travelTotal,
    installationTotal,
  ...
  // Calculate actual rates and quantities from the data
  let labourRate = 25.00; // Default rate
  let labourHours = 8; // Default hours
  let materialsRate = 0.00;
  let materialsQuantity = 1;
  let travelRate = 0.00;
  let travelDistance = 10;

  // Try to extract actual rates from the data
  if (data.labourRate && data.labourHours) {
    labourRate = parseFloat(data.labourRate) || 25.00;
    labourHours = parseFloat(data.labourHours) || 8;
  } else if (labourTotal > 0) {
    // Calculate rate from total and hours, or hours from total and rate
    if (data.labourHours) {
      labourHours = parseFloat(data.labourHours);
      labourRate = labourTotal / labourHours;
    } else if (data.labourRate) {
      labourRate = parseFloat(data.labourRate);
      labourHours = labourTotal / labourRate;
    }
  }

  if (data.materialRate && data.materialSQM) {
    materialsRate = parseFloat(data.materialRate) || 0.00;
    materialsQuantity = parseFloat(data.materialSQM) || 1;
  } else if (data.materialsCost && data.materialsQuantity) {
    materialsRate = parseFloat(data.materialsCost) || 0.00;
    materialsQuantity = parseFloat(data.materialsQuantity) || 1;
  } else if (data.MaterialsCost && data.MaterialsQuantity) {
    materialsRate = parseFloat(data.MaterialsCost) || 0.00;
    materialsQuantity = parseFloat(data.MaterialsQuantity) || 1;
  } else if (materialsTotal > 0) {
    materialsRate = materialsTotal; // Use total as rate for display
    materialsQuantity = 1;
  }

  if (data.travelCost && data.travelDistance) {
    travelRate = parseFloat(data.travelCost) || 0.00;
    travelDistance = parseFloat(data.travelDistance) || 10;
  } else if (travelTotal > 0) {
    travelRate = travelTotal / 10; // Assume 10km default distance
    travelDistance = 10;
  }

  // Detailed breakdown - use calculated values
  html = html.replace(/{{LABOUR_RATE}}/g, formatCurrency(labourRate));
  html = html.replace(/{{LABOUR_HOURS}}/g, labourHours.toFixed(2));
  html = html.replace(/{{LABOUR_TOTAL}}/g, formatCurrency(labourTotal));

  html = html.replace(/{{MATERIALS_RATE}}/g, formatCurrency(materialsRate));
  html = html.replace(/{{MATERIALS_UNIT}}/g, 'unit');
  html = html.replace(/{{MATERIALS_QUANTITY}}/g, materialsQuantity.toFixed(2));
  html = html.replace(/{{MATERIALS_TOTAL}}/g, formatCurrency(materialsTotal));

  html = html.replace(/{{TRAVEL_RATE}}/g, formatCurrency(travelRate));
  html = html.replace(/{{TRAVEL_DISTANCE}}/g, travelDistance.toFixed(2));
  html = html.replace(/{{TRAVEL_TOTAL}}/g, formatCurrency(travelTotal));

  html = html.replace(/{{INSTALLATION_TOTAL}}/g, formatCurrency(installationTotal));

  // Totals
  html = html.replace(/{{SUBTOTAL}}/g, formatCurrency(subtotal));
  html = html.replace(/{{GST}}/g, formatCurrency(gst));
  html = html.replace(/{{FINAL_TOTAL}}/g, formatCurrency(finalTotal));

  // Total SQM
  html = html.replace(/{{TOTAL_SQM}}/g, data.totalSqm || data.TOTAL_SQM || '0.00');
  ...
  if (data.rooms) {
    try {
      ...
      console.warn('Failed to parse rooms data:', e);
      rooms = [];
    }
  }
  ...
  const roomRows = rooms.map(room => {
    ...
    return `
      <tr>
        <td>${room.name || 'N/A'}</td>
        <td>${sqm.toFixed(2)} sqm</td>
      </tr>
    `;
  }).join('');
  
  html = html.replace('{{ROOM_ROWS}}', roomRows || '<tr><td colspan="2" style="text-align: center;">No room details available</td></tr>');

  // Choose PDF provider based on usage
  const usage = await getCurrentPDFUsage();
  const provider = selectPDFProvider(usage);

  let pdfBuffer;

  if (provider === 'PDFShift') {
    pdfBuffer = await convertWithPDFShift(html);
  } else {
    pdfBuffer = await convertWithAPI2PDF(html);
  }

  // Track usage after successful generation
  if (pdfBuffer) {
    await trackPDFUsage(provider);
  }

  return pdfBuffer;
}

function normalizeInstallationData(data) {
  // Create a simple normalization function for installation cost
  const cost = data.InstallationCost || data.installationCost || data.installation_cost || 0;
...
function fillTemplate(template, data) {
  ...
  // Show only suburb and area, not full street address
  const addressParts2 = (data.customerAddress || '').split(',');
  const suburbArea2 = addressParts2.length >= 2 ? 
    `${addressParts2[addressParts2.length - 2]?.trim()}, ${addressParts2[addressParts2.length - 1]?.trim()}` : 
    (data.customerAddress || 'N/A');
  html = html.replace(/{{CUSTOMER_ADDRESS}}/g, suburbArea2);
  ...
  const totalSqm = parseFloat(data.totalSqm) || 0;
  ...
  html = html.replace(/{{LABOUR_HOURS}}/g, labourData.hours.toFixed(2));
  html = html.replace(/{{MATERIALS_RATE}}/g, safeFormatCurrency(materialsData.cost));
  html = html.replace(/{{MATERIALS_QUANTITY}}/g, materialsData.quantity.toFixed(2));
    html = html.replace(/{{MATERIALS_UNIT}}/g, 'sqm');
  html = html.replace(/{{TRAVEL_RATE}}/g, safeFormatCurrency(travelData.cost));
  html = html.replace(/{{TRAVEL_DISTANCE}}/g, travelData.distance.toFixed(2));
  html = html.replace(/{{TOTAL_SQM}}/g, totalSqm.toFixed(2));
...
function generateRoomRows(rooms, roomPricingData = null) {
  ...
  return rooms.map((room, index) => {
    let sqm = parseFloat(room.sqm) || 0;
    if (sqm === 0 && room.width && room.length) {
      sqm = parseFloat(room.width) * parseFloat(room.length);
    }
    const dimensions = room.dimensions || (room.width && room.length ? `${room.width}m x ${room.length}m` : room.originalInput) || 'N/A';
...
async function convertHTMLToPDF(html) {
  ...
  // PDFShift if under quota and API key available
  if (process.env.PDFSHIFT_API_KEY && usage.pdfshiftCount < 50) {
    providers.push('PDFShift');
  }
  
  // API2PDF as final fallback if API key available
  ...
  if (providers.length === 0) {
    ...
    throw new Error('PDF generation requires API service configuration (PDFShift or API2PDF)');
  }
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Quote ${data.quoteId} - Heat.nz</title>
    <style>
  ...
  try {
    ...
    const tradespersonParagraphs = [
      ...
      new Paragraph({
        children: [
          new TextRun({ text: "Name: ", bold: true }),
          new TextRun({ text: data.TradePersonName || 'N/A' }),
        ],
      }),
      new Paragraph({
        children: [
          new TextRun({ text: "Email: ", bold: true }),
          new TextRun({ text: data.tradePersonEmail || 'N/A' }),
        ],
      }),
      new Paragraph({
        children: [
          new TextRun({ text: "Phone: ", bold: true }),
          new TextRun({ text: data.tradePersonPhone || 'N/A' }),
        ],
      }),
      new Paragraph({
        children: [
          new TextRun({ text: "License: ", bold: true }),
          new TextRun({ text: data.tradePersonLicense || 'N/A' }),
        ],
      }),
      new Paragraph({ text: "" }), // Space
    ];
    ...
    const termsParagraphs = [
      ...
      new Paragraph({ text: "" }), // Space
      new Paragraph({
        text: "Thank you for choosing Heat.nz for your underfloor heating needs.",
        alignment: AlignmentType.CENTER,
      }),
    ];
