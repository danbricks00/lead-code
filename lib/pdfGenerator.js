...
export async function generateQuotePDF(quoteData) {
  ...
  const suburb = quoteData.Suburb || quoteData.suburb;
  const area = quoteData.Area || quoteData.area;
  const addressParts = [suburb, area].filter(Boolean);
  let customerAddress = addressParts.join(', ');

  if (!customerAddress) {
    customerAddress = quoteData.CustomerAddress || quoteData.Location || 'N/A';
  }
  ...
  const roomRows = rooms.map(room => {
    const roomLabour = parseFloat(room.labour) || 0;
    const roomMaterials = parseFloat(room.materials) || 0;
    const roomTotal = roomLabour + roomMaterials;
    return `
      <tr>
        <td>${room.name || 'N/A'}</td>
        <td>${room.size || 'N/A'}</td>
        <td>${formatCurrency(roomLabour)}</td>
        <td>${formatCurrency(roomMaterials)}</td>
        <td>${formatCurrency(roomTotal)}</td>
      </tr>
    `;
  }).join('');
  html = html.replace('{{ROOM_ROWS}}', roomRows || '<tr><td colspan="5" style="text-align: center;">No room details available</td></tr>');
