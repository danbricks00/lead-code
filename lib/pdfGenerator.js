...
export async function generateQuotePDF(quoteData) {
  // This function now receives a combined object of lead and quote data

  // Replace placeholders with actual data
  let html = getQuoteTemplate();

  // Use the robust normalization functions to get the data
  const labour = normalizeLabourData(quoteData);
  const materials = normalizeMaterialsData(quoteData);
  const travel = normalizeTravelData(quoteData);
  const installation = normalizeInstallationData(quoteData); // Assuming this function exists or needs to be created

  const customerAddress = quoteData.Address || [quoteData.Suburb, quoteData.Area].filter(Boolean).join(', ') || quoteData.CustomerAddress || quoteData.Location || 'N/A';

  // Customer details - use quoteData directly as it contains lead info
  html = html.replace(/{{CUSTOMER_NAME}}/g, quoteData.CustomerName || 'N/A');
  html = html.replace(/{{CUSTOMER_EMAIL}}/g, quoteData.CustomerEmail || 'N/A');
  html = html.replace(/{{CUSTOMER_PHONE}}/g, quoteData.CustomerPhone || 'N/A');
  html = html.replace(/{{CUSTOMER_ADDRESS}}/g, customerAddress);
  html = html.replace(/{{SERVICE_TYPE}}/g, quoteData.ServiceType || 'Underfloor Heating');

  // Tradesperson details
  html = html.replace(/{{TRADESPERSON_NAME}}/g, quoteData.TradePersonName || 'N/A');
  html = html.replace(/{{TRADESPERSON_EMAIL}}/g, quoteData.TradePersonEmail || 'N/A');
  html = html.replace(/{{TRADESPERSON_PHONE}}/g, quoteData.TradePersonPhone || 'N/A');
  html = html.replace(/{{TRADESPERSON_LICENSE}}/g, quoteData.TradePersonLicense || 'N/A');

  // Quote details
  html = html.replace(/{{QUOTE_ID}}/g, quoteData.quoteId || quoteData.QuoteID || 'N/A');
  html = html.replace(/{{QUOTE_DATE}}/g, formatDate(quoteData.Date || new Date()));
  html = html.replace(/{{VALID_UNTIL}}/g, formatDate(quoteData.ValidUntil));

  // Detailed breakdown
  html = html.replace(/{{LABOUR_RATE}}/g, formatCurrency(labour.rate));
  html = html.replace(/{{LABOUR_HOURS}}/g, labour.hours);
  html = html.replace(/{{LABOUR_TOTAL}}/g, formatCurrency(labour.total));

  html = html.replace(/{{MATERIALS_RATE}}/g, formatCurrency(materials.rate));
  html = html.replace(/{{MATERIALS_UNIT}}/g, materials.unit || 'unit');
  html = html.replace(/{{MATERIALS_QUANTITY}}/g, materials.quantity);
  html = html.replace(/{{MATERIALS_TOTAL}}/g, formatCurrency(materials.total));

  html = html.replace(/{{TRAVEL_RATE}}/g, formatCurrency(travel.rate));
  html = html.replace(/{{TRAVEL_DISTANCE}}/g, travel.distance);
  html = html.replace(/{{TRAVEL_TOTAL}}/g, formatCurrency(travel.total));

  html = html.replace(/{{INSTALLATION_TOTAL}}/g, formatCurrency(installation.total));

  // Totals
  const subtotal = labour.total + materials.total + travel.total + installation.total;
  ...
  // Room details
  const rooms = quoteData.Rooms ? JSON.parse(quoteData.Rooms) : [];
  const roomRows = rooms.map(room => {
    // Calculate total sqm for the room
    const width = parseFloat(room.width) || 0;
    const length = parseFloat(room.length) || 0;
    const totalSqm = (width * length).toFixed(2);
