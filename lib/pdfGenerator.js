...
export async function generateQuotePDF(quoteData) {
  ...
  try {
    ...
    let html = fillTemplate(htmlTemplate, data);
    
    // Generate room rows for the project details table
    let rooms = [];
    if (data.rooms) {
      try {
    ...
    // FIXED: Ensure rooms is an array before calling map
    const roomRows = Array.isArray(rooms) ? 
      rooms.map(room => {
        const sqm = parseFloat(room.sqm) || 0;
        return `
        <tr>
    ...
    // Use the robust convertHTMLToPDF function that handles fallbacks
    return await convertHTMLToPDF(html);
    
  } catch (error) {
    console.error('❌ PDF generation failed:', error);
    throw error; // Re-throw the error to be handled by quote-submit.js
  }
}
