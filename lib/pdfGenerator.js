...
export async function generateQuotePDF(quoteData) {
  ...
  try {
    ...
    if (quoteData.pdfData) {
      ...
      // Ensure rooms is always an array to prevent mapping errors
        rooms: []
      };
    } else {
    ...
    // Since room data isn't needed on the PDF, replace ALL occurrences of the placeholder
    // Use a regular expression with the global flag to replace all occurrences
    html = html.replace(/\{\{ROOM_ROWS\}\}/g, '<tr><td colspan="5" style="text-align: center;">Room details available on quote submission page</td></tr>');
