...
async function convertHTMLToPDF(html) {
  ...
  console.log('   Adobe PDF Organization ID:', process.env.ADOBE_PDF_ORGANIZATION_ID ? '✅ Set' : '❌ Missing');
  ...
  // 1. Adobe first if under quota and credentials available
...
async function convertWithAdobePDF(html) {
  ...
  try {
    ...
    if (process.env.ADOBE_PDF_ORGANIZATION_ID) {
      headers['x-gw-ims-org-id'] = process.env.ADOBE_PDF_ORGANIZATION_ID;
    }
