...
export async function generateQuotePDF(quoteData) {
  ...
  // Financial data - use actual calculated values
  const labourTotal = parseFloat(data.labourTotal) || 0;
  const materialsTotal = parseFloat(data.materialsTotal) || 0;
  const travelTotal = parseFloat(data.travelTotal) || 0;
  const installationTotal = parseFloat(data.installationCost) || 0;
  const subtotal = parseFloat(data.subtotal) || (labourTotal + materialsTotal + travelTotal + installationTotal);
  const gst = parseFloat(data.gst) || (subtotal * 0.15);
  const finalTotal = parseFloat(data.total) || (subtotal + gst);
