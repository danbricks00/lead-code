...
function fillTemplate(template, data) {
  ...
  if (totalSqm === 0 && data.rooms && data.rooms.length > 0) {
    totalSqm = data.rooms.reduce((acc, room) => {
      let roomSqm = parseFloat(room.sqm) || 0;
      if (roomSqm === 0 && room.width && room.length) {
        roomSqm = parseFloat(room.width) * parseFloat(room.length);
      }
      return acc + roomSqm;
    }, 0);
  }
