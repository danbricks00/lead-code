...
/** Lead / tradesperson inbox: TRADESPERSON_EMAIL, then ADMIN_EMAIL so enquiries are not dropped. */
export function getTradespersonLeadEmail() {
  const raw = process.env.TRADESPERSON_EMAIL || process.env.ADMIN_EMAIL || '';
...
export async function sendEmail(options) {
  try {
    ...
    // Add BCC for tracking if CODER_EMAIL is set (unless BCC is already explicitly set)
    const coderEmail = process.env.CODER_EMAIL;
    if (coderEmail && !emailOptions.bcc) {
    ...
    const bccInfo = coderEmail && !options?.bcc ? ` (BCC: ${coderEmail})` : '';
    console.log(`✅ Email sent to ${recipients}${bccInfo}, msgId: ${result.messageId}`);
