...
try {
    ...
    if (addedFiles.length > 0) {
        ...
        const summary = `Add ${addedFiles.length > 1 ? 'files' : 'file'}: ${newFiles.join(', ')}`;
    ...
    if (modifiedFiles.length > 0) {
        ...
        const summary = `Update ${modifiedFiles.length > 1 ? 'files' : 'file'}: ${changedFiles.join(', ')}`;
    ...
    if (deletedFiles.length > 0) {
        ...
        const summary = `Remove ${deletedFiles.length > 1 ? 'files' : 'file'}: ${removedFiles.join(', ')}`;
