...
export async function generateQuotePDF(quoteData) {
  ...
  // Detailed breakdown - use actual values instead of normalization
  html = html.replace(/{{LABOUR_RATE}}/g, formatCurrency(25.00)); // Default rate
  html = html.replace(/{{LABOUR_HOURS}}/g, '8'); // Default hours
  html = html.replace(/{{LABOUR_TOTAL}}/g, formatCurrency(labourTotal));

  // Calculate proper rates for materials and travel
  const materialsRate = materialsTotal > 0 ? materialsTotal : 0.00;
  const travelRate = travelTotal > 0 ? (travelTotal / 10) : 0.00; // Assuming 10km default distance

  html = html.replace(/{{MATERIALS_RATE}}/g, formatCurrency(materialsRate));
  html = html.replace(/{{MATERIALS_UNIT}}/g, 'unit');
  html = html.replace(/{{MATERIALS_QUANTITY}}/g, '1');
  ...
  html = html.replace(/{{TRAVEL_DISTANCE}}/g, '10');
