...
async function initializeXero() {
  try {
    ...
    // Manually create a TokenSet instance and assign it
    xero.tokenSet = new TokenSet(tokenSet);

    // --- START DIAGNOSTIC LOGGING ---
    console.log('[XERO-DIAGNOSTIC] Type of xero.tokenSet:', typeof xero.tokenSet);
    if (xero.tokenSet) {
        console.log('[XERO-DIAGNOSTIC] Value of xero.tokenSet:', JSON.stringify(xero.tokenSet, null, 2));
        console.log('[XERO-DIAGNOSTIC] Does xero.tokenSet have expired method?', typeof xero.tokenSet.expired);
    } else {
        console.log('[XERO-DIAGNOSTIC] xero.tokenSet is null or undefined.');
    }
