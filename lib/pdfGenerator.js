...
<body>
    <div class="quote-container">
        ...
        <div class="breakdown-section" style="margin-bottom: 30px;">
            <div class="summary-title">Detailed Breakdown</div>
            <table class="breakdown-table" style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                ...
                <tr>
                    <td style="border: 1px solid #ddd; padding: 12px;"><strong>Labour</strong></td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">${{LABOUR_RATE}}/hour</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">{{LABOUR_HOURS}} hours</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: right; font-weight: bold;">${{LABOUR_TOTAL}}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #ddd; padding: 12px;"><strong>Materials</strong></td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">${{MATERIALS_RATE}}/{{MATERIALS_UNIT}}</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">{{MATERIALS_QUANTITY}} {{MATERIALS_UNIT}}</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: right; font-weight: bold;">${{MATERIALS_TOTAL}}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #ddd; padding: 12px;"><strong>Travel</strong></td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">${{TRAVEL_RATE}}/km</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">{{TRAVEL_DISTANCE}} km</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: right; font-weight: bold;">${{TRAVEL_TOTAL}}</td>
                </tr>
                <tr>
                    <td style="border: 1px solid #ddd; padding: 12px;"><strong>Installation & Setup</strong></td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">-</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: center;">1 job</td>
                    <td style="border: 1px solid #ddd; padding: 12px; text-align: right; font-weight: bold;">${{INSTALLATION_TOTAL}}</td>
                </tr>
            </table>
            
            <!-- Total Square Meters -->
            <div style="background-color: #e8f4f8; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                <strong>Total Project Area: {{TOTAL_SQM}} square meters</strong>
            </div>
        </div>
...
function fillTemplate(template, data) {
  ...
  // Date formatting function (moved to top for scope)
  ...
  // Currency formatting function (moved to top for scope)
  ...
  // Detailed breakdown replacements with enhanced logging
  console.log('🔍 Processing breakdown placeholders...');
  if (data.breakdown) {
    console.log('✅ Breakdown data available:', Object.keys(data.breakdown));
    html = html.replace(/{{LABOUR_RATE}}/g, formatCurrency(data.breakdown.labourRate || 0));
    html = html.replace(/{{LABOUR_HOURS}}/g, (data.breakdown.labourHours || 0).toFixed(1));
    html = html.replace(/{{MATERIALS_RATE}}/g, formatCurrency(data.breakdown.materialsCost || 0));
    html = html.replace(/{{MATERIALS_QUANTITY}}/g, (data.breakdown.materialsQuantity || 0).toString());
    html = html.replace(/{{MATERIALS_UNIT}}/g, 'sqm');
    html = html.replace(/{{TRAVEL_RATE}}/g, formatCurrency(data.breakdown.travelCost || 0));
    html = html.replace(/{{TRAVEL_DISTANCE}}/g, (data.breakdown.travelDistance || 0).toString());
    html = html.replace(/{{TOTAL_SQM}}/g, (data.breakdown.totalSqm || 0).toFixed(1));
    console.log('✅ All breakdown placeholders processed');
  } else {
    console.log('⚠️ No breakdown data, using fallback values');
    // Fallback values if breakdown is not available
    html = html.replace(/{{LABOUR_RATE}}/g, '0.00');
    html = html.replace(/{{LABOUR_HOURS}}/g, '0.0');
    html = html.replace(/{{MATERIALS_RATE}}/g, '0.00');
    html = html.replace(/{{MATERIALS_QUANTITY}}/g, '0');
    html = html.replace(/{{MATERIALS_UNIT}}/g, 'sqm');
    html = html.replace(/{{TRAVEL_RATE}}/g, '0.00');
    html = html.replace(/{{TRAVEL_DISTANCE}}/g, '0');
    html = html.replace(/{{TOTAL_SQM}}/g, '0.0');
  }
